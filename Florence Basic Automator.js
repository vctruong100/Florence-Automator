...
(function () {
    ...
    function buildCandidatePairKeys(name) {
        ...
        if (filtered.length === 1) {
            ...
            return { searchTerm: word1, pairKeys: [singleKey] };
        }
        ...
        if (filtered.length === 2) {
            ...
            return { searchTerm: word1, pairKeys: [twoWordKey] };
        }
        var word3 = filtered[2];
        var pk1 = elogNormalizeName(word1 + ' ' + word2);
        var pk2 = elogNormalizeName(word1 + ' ' + word3);
        var pairKeys = [pk1];
        if (pk2 !== pk1) {
            pairKeys.push(pk2);
        }
        addLogMessage('[Florence] buildCandidatePairKeys: three+ words, searchTerm="' + word1 + '" pairKeys=[' + pairKeys.join(', ') + ']', 'log');
        return { searchTerm: word1, pairKeys: pairKeys };
    }
    ...
    function attemptSelectByScrollingForName(targetDisplay, targetPairKey, candidatePairKeys) {
        var candidates = candidatePairKeys && candidatePairKeys.length > 0 ? candidatePairKeys : buildCandidatePairKeys(targetDisplay).pairKeys;
        var searchInfo = buildCandidatePairKeys(targetDisplay);
        var firstName = searchInfo.searchTerm;
        var nameParts = getFirstAndLast(targetDisplay);
        var lastName = nameParts[1];
        addLogMessage('[Florence] attemptSelectByScrollingForName: target=' + targetDisplay + ' firstName=' + firstName + ' lastName=' + lastName + ' candidates=[' + candidates.join(', ') + ']', 'log');
        return new Promise(function(resolve) {
            ...
            var firstNameTid = setTimeout(function() {
                ...
                reopenDropdownIfClosed(elogState).then(function() {
                    var match = scanFilteredOptionsForMatch(targetPairKey, ELOG_FORM_SELECTORS, candidates);
                    if (match) {
                    ...
                    if (lastName && lastName !== firstName) {
                        ...
                        var clearTid1 = setTimeout(function() {
                            ...
                            var lastNameTid = setTimeout(function() {
                                ...
                                reopenDropdownIfClosed(elogState).then(function() {
                                    var match2 = scanFilteredOptionsForMatch(targetPairKey, ELOG_FORM_SELECTORS, candidates);
                                    if (match2) {
                                    ...
                                    addLogMessage('attemptSelectByScrollingForName: not found with lastName filter, marking as not found', 'log');
                                    clearFilteredInput(inputEl);
                                    resolve(false);
                                });
                        ...
                        addLogMessage('attemptSelectByScrollingForName: lastName same as firstName, marking as not found', 'log');
                        clearFilteredInput(inputEl);
                        resolve(false);
                    }
                });
...
function showResponsibilitiesProgressPanel(rolesData) {
    ...
    function attemptDoASelectByScrollingForName(targetDisplay, targetPairKey, candidatePairKeys) {
        ...
        var nameParts = getFirstAndLast(targetDisplay);
        var lastName = nameParts[1];
        addLogMessage('[Florence] attemptDoASelectByScrollingForName: target=' + targetDisplay + ' firstName=' + firstName + ' lastName=' + lastName + ' candidates=[' + candidates.join(', ') + ']', 'log');
        return new Promise(function(resolve) {
            var inputEl = document.querySelector(DOA_SELECTORS.memberInput);
            if (!inputEl) {
            ...
            var firstNameTid = setTimeout(function() {
                ...
                reopenDropdownIfClosed(doaState).then(function() {
                    var match = scanFilteredOptionsForMatch(targetPairKey, DOA_SELECTORS, candidates);
                    if (match) {
                    ...
                    if (lastName && lastName !== firstName) {
                        ...
                        var clearTid1 = setTimeout(function() {
                            ...
                            var lastNameTid = setTimeout(function() {
                                ...
                                reopenDropdownIfClosed(doaState).then(function() {
                                    var match2 = scanFilteredOptionsForMatch(targetPairKey, DOA_SELECTORS, candidates);
                                    if (match2) {
                                    ...
                                    addLogMessage('attemptDoASelectByScrollingForName: not found with lastName filter, marking as not found', 'log');
                                    clearFilteredInput(inputEl);
                                    resolve(false);
                                });
                        ...
                        addLogMessage('attemptDoASelectByScrollingForName: lastName same as firstName, marking as not found', 'log');
                        clearFilteredInput(inputEl);
                        resolve(false);
                    }
                });
